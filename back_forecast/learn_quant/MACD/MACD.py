...
# 更新股票池的间隔天数
CHANGE_STOCK_POOL_DAY_NUMBER = 250
...
def init_global(context):
    ...
    '''
    # 距上一次股票池更新的天数
    context.stock_pool_update_day = 0
    # 股票池，股票代码
    context.stock_pool = []
    # 距离上一次处理交易逻辑的bar的个数
    context.bar_number = 0
    # 每只标的需要买入的头寸
    context.money_value = context.portfolio.cash / 20

    # 缓存15分钟级别计算MACD需要的数据
    context.macd_cache_long           = None
    # 缓存5分钟级别计算MACD需要的数据
    context.macd_cache_short          = None
    # 缓存15分钟级别底背离数据
    context.bottom_divergence_long    = dict()
    # 缓存15分钟级别顶背离数据
    context.top_divergence_long       = dict()
    # 缓存5分钟级别底背离数据
    context.bottom_divergence_short   = dict()
    # 缓存5分钟级别顶背离数据
    context.top_divergence_short      = dict()
...
def handle_bar(context,  bar_dict):
    ...
    if context.macd_cache_long is not None:
        # 15分钟级别交易
        if context.bar_number % LONG_TRADE_BAR_DURATION == 0:
            update_macd_cache(context, LONG_UNIT)
            buy(context, LONG_UNIT)   # 买入
            sell(context,LONG_UNIT)  # 卖出
    # 5分钟级别交易
    if context.macd_cache_short is not None:
        if context.bar_number % SHORT_TRADE_BAR_DURATION == 0:
            print("current time :", context.now)
            update_macd_cache(context, SHORT_UNIT)
            buy(context, SHORT_UNIT)   # 买入
            sell(context,SHORT_UNIT)  # 卖出
    context.bar_number = context.bar_number + 1
    pass
...
def update_macd_cache(context, unit):
    ...
    更新MACD缓存数据
    '''
    current_tm = context.now.strftime("%Y-%m-%d %H:%M:%S")
    if unit == LONG_UNIT:
        # 更新15分钟级别的数据
        context.macd_cache_long.update_cache(context.now)
        # 15分钟级别不需要判断连续背离，所以不需要缓存过多的数据
        context.bottom_divergence_long = dict()
        context.top_divergence_long = dict()

        for code in context.macd_cache_long.bars.keys():
            if code not in context.bottom_divergence_long.keys():
                context.bottom_divergence_long[code] = dict()
            if code not in context.top_divergence_long.keys():
                context.top_divergence_long[code] = dict()

            # 获取最新一根bar检测到的背离
            divergences = context.macd_cache_long.divergences[code] if code in context.macd_cache_long.divergences.keys() else []
            if len(divergences) > 0:
                for divergence in divergences:
                    ...
                    if divergence.divergence_type == DivergenceType.Bottom:
                        if current_tm not in context.bottom_divergence_long[code]:
                            context.bottom_divergence_long[code][current_tm] = list()

                        context.bottom_divergence_long[code][current_tm].append({
                        ...
                        # 顶背离
                        if current_tm not in context.top_divergence_long[code]:
                            context.top_divergence_long[code][current_tm] = list()
                        context.top_divergence_long[code][current_tm].append({
                            "last_dif_limit_tm": last_dif_limit_tm,
                            "pre_dif_limit_tm": divergence.pre_dif_limit_tm.strftime("%Y-%m-%d %H:%M:%S")
                            })
    elif unit == SHORT_UNIT:
        # 更新5分钟级别的数据
        context.macd_cache_short.update_cache(context.now)

        for code in context.macd_cache_short.bars.keys():
            if code not in context.bottom_divergence_short.keys():
                context.bottom_divergence_short[code] = dict()
            if code not in context.top_divergence_short.keys():
                context.top_divergence_short[code] = dict()

            # 获取最新一根bar检测到的背离
            divergences = context.macd_cache_short.divergences[code] if code in context.macd_cache_short.divergences.keys() else []
            if len(divergences) > 0:
                for divergence in divergences:
                    last_dif_limit_tm = divergence.last_dif_limit_tm.strftime("%Y-%m-%d %H:%M:%S")
                    # DivergenceType.Bottom - 底背离，DivergenceType.Top - 顶背离
                    if divergence.divergence_type == DivergenceType.Bottom:
                        if current_tm not in context.bottom_divergence_short[code]:
                            context.bottom_divergence_short[code][current_tm] = list()

                        context.bottom_divergence_short[code][current_tm].append({
                            "last_dif_limit_tm":last_dif_limit_tm,
                            "pre_dif_limit_tm":divergence.pre_dif_limit_tm.strftime("%Y-%m-%d %H:%M:%S")
                            })
                        ...
                        # 顶背离
                        if current_tm not in context.top_divergence_short[code]:
                            context.top_divergence_short[code][current_tm] = list()
                        context.top_divergence_short[code][current_tm].append({
                            "last_dif_limit_tm":last_dif_limit_tm,
                            "pre_dif_limit_tm":divergence.pre_dif_limit_tm.strftime("%Y-%m-%d %H:%M:%S")
                            })
    pass
...
def after_market_close(context,bar_dict):
    ...
    2. 更新MACD缓存数据
    '''
    if context.stock_pool_update_day % CHANGE_STOCK_POOL_DAY_NUMBER == 0:
        # 更新股票池
        stock_pool(context)
        stock_pools = set()
        ...
        # 缓存15分钟级别计算MACD需要的数据
        context.macd_cache_long  = MacdCache(LONG_UNIT,  context.now, count=250, stocks=stock_pools)
        # 缓存5分钟级别计算MACD需要的数据
        context.macd_cache_short = MacdCache(SHORT_UNIT, context.now, count=250, stocks=stock_pools)
        log.info("缓存计算MACD需要使用到的数据完毕")
    context.stock_pool_update_day = (context.stock_pool_update_day + 1) % CHANGE_STOCK_POOL_DAY_NUMBER
...
def close_position(context):
    ...
    '''
    for code in context.portfolio.positions.keys():
        if code not in context.stock_pool:
            # if is_low_limit(code):
            #     continue
            # 标的已经不在股票池中尝试卖出该标的的股票
            order_ = order_target_value(code, 0)
...
def sell(context,unit):
    ...
    '''
    for code in context.portfolio.positions.keys():
        current_data = context.portfolio.positions[code]
        if current_data is None:
            return
        # if is_low_limit(code):
        #     continue
        if context.portfolio.positions[code].quantity <= 0:
            continue
        ...
        if unit == LONG_UNIT:
            # 15分钟级别交易顶背离卖出
            if current_tm in context.top_divergence_long[code].keys():
                is_sell     = True
                sell_type   = "LONG "
        elif unit == SHORT_UNIT:
            # 5分钟级别交易 连续顶背离卖出
            if current_tm in context.top_divergence_short[code].keys():
                # 发生了5分钟级别的底背离，需要判断是否发生了连续的底背离
                for i in range(len(context.top_divergence_short[code][current_tm])):
                    item_obj = context.top_divergence_short[code][current_tm][i]
                    pre_dif_limit_tm = item_obj['pre_dif_limit_tm']

                    for time_key in context.top_divergence_short[code].keys():
                        if is_sell:
                            break
                        for j in range(len(context.top_divergence_short[code][time_key])):
                            last_dif_limit_tm = context.top_divergence_short[code][time_key][j]
...
def buy(context,unit):
    ...
    for code in context.stock_pool:
        ...
        if unit == LONG_UNIT:
            # 15分钟级别交易底背离买入
            if current_tm in context.bottom_divergence_long[code].keys():
                # 发生了15分钟级别的底背离
                is_buy = True
                buy_type = "LONG "
        elif unit == SHORT_UNIT:
            # 5分钟级别交易 连续底背离买入
            if current_tm in context.bottom_divergence_short[code].keys():
                # 发生了5分钟级别的底背离，需要判断是否发生了连续的底背离
                for i in range(len(context.bottom_divergence_short[code][current_tm])):
                    item_obj = context.bottom_divergence_short[code][current_tm][i]
                    pre_dif_limit_tm = item_obj['pre_dif_limit_tm']

                    for time_key in context.bottom_divergence_short[code].keys():
                        if is_buy:
                            break
                        for j in range(len(context.bottom_divergence_short[code][time_key])):
                            last_dif_limit_tm = context.bottom_divergence_short[code][time_key][j]
                            if pre_dif_limit_tm == last_dif_limit_tm:
                                is_buy = True
                                buy_type = "SHORT"
                                break
        if is_buy:
            print("买入")
            order_ = order_target_value(code, context.money_value)
            if (order_ is not None) and (order_.filled > 0):
...
def stock_pool(context):
    context.stock_pool = ['000651.XSHE']
    pass
...
#         context.stock_pool.append(stock['code'])
#     log.info('调整股票池,筛选出的股票池：',context.stock_pool)
#     pass
