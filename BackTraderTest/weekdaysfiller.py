...
class WeekDaysFiller(object):
    ...
    def __init__(self, data, cal_date, fillclose=False):
    ...
    def __call__(self, data):
        ...
        if dt not in self.cal_date.index:
            print('--------', dt)
            data.backwards()
            return False

        while lastdt < dt:  # loop over gap bars
            if lastdt in self.cal_date.index:
                ...
                self.voidbar[data.DateTime] = data.date2num(dtime)
